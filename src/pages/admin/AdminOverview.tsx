  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const results = await Promise.all([
        ...
        supabase.from('bounties').select('*', { count: 'exact', head: true }).eq('status', 'approved').eq('is_closed', false).eq('is_deleted', false),
        supabase.from('bounties').select('*', { count: 'exact', head: true }).eq('is_closed', true).eq('is_deleted', false),
